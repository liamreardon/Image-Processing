...
public class GaussianFilter {
	...
	public void init(int[] original, int sigmaIn, int tempSize, int widthIn, int heightIn) {
		...
		if((tempSize%2)==0) {
			this.templateSize=tempSize-1;
		}
		else {
			this.templateSize = tempSize;
		}
		this.sigma=(double)sigmaIn;
		this.width=widthIn;
		this.height=heightIn;
		this.input = new int[width*height];
		this.output = new int[width*height];
		this.template = new float[templateSize*templateSize];
		this.input=original;
	}
			...
			for(int y=(templateSize-1)/2; y<height-(templateSize+1)/2;y++) {
				...
				for(int x1=0;x1<templateSize;x1++) {
					for(int y1=0;y1<templateSize;y1++) {
						...
						float value = ((int)input[y2*width+x2] & 0xff) * (template[y1*templateSize+x1]);
						sum += value;
					}
				}
